...
import { useGetBookingQuery } from "../../api/bookingApi";
...
const Profile = ({ selectedLanguage }) => {
  ...
  const { data, isError, isLoading, isSuccess, refetch } = useGetBookingQuery({
    lange: language,
    token,
  });
  ...
  const {
    data: inqueryData,
    isError: isInqueryError,
    isLoading: isInqueryLoading,
    isSuccess: isInquerySuccess,
    refetch: inqueryRefetch,
  } = useGetInqueryQuery({ lang: language, token });

  let inqueryContent = null;

  if (isInqueryError) {
    inqueryContent = <>Inquery Data Fetch Error</>;
  }
  ...
  useEffect(() => {
    ...
    const data = JSON.parse(localStorage.getItem("real-estate-auth-info"));
    if (data) {
